...
from ...whisper_main.translate2text import translate2text
from gtts import tts


def main(args, ko_audio: input):
    ...
    # Korean Voice to English Text file by using 'whisper'
    en_text = translate2text(ko_audio)

    # English Audio to English Text by using 'gTTS'
    en_audio = tts(en_text)

    # 2. Predict the gestures with the loaded model
    # motion = gp.predict_gestures(args.audio, args.text)
    motion = gp.predict_gestures(en_audio, en_text)
